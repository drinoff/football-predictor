...
const MatchDetail = ({ matchDetail, h2h }) => {
  return (
    <>
      ...
      <div className="refereeAndDate">
        <p className="eventDate">
          {matchDetail.fixture.date.substring(0, 16).replace("T", " - ")}
        </p>
        <p className="eventReferee">
          {matchDetail.fixture.referee
            ? "Referee: " + matchDetail.fixture.referee
            : "Referee: No Info"}
        </p>
      </div>
      <div className="h2hChart">
        {h2h ? <Chart h2h={h2h} teams={matchDetail.teams} /> : ""}
      </div>
    </>
  );
};
